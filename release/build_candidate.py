#!/usr/bin/env python3
"""Build one deterministic, candidate-only Recorder Next rev17 generation."""
...
def copy_product(repo: Path, candidate: Path) -> list[dict[str, Any]]:
    ...
    if len(rows) != 44:
        raise BuildError(f"product candidate has {len(rows)} files, expected 44")
    return rows
...
def build(repo: Path, output: Path) -> Path:
    ...
    if product_test_count != 84:
        raise BuildError(f"product suite ran {product_test_count}, expected 84")
